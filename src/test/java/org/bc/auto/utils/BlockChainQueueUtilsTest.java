...
public class BlockChainQueueUtilsTest {

    private static final Logger logger = LoggerFactory.getLogger(BlockChainQueueUtilsTest.class);
    ...
    public void testAdd(){
        for(int i=0;i<10;i++){
            ...
            BlockChainQueueUtils.add(bcCluster);
            logger.info("队列的长度为：{}",BlockChainQueueUtils.size());
        }
    }
    ...
    public void testRemove()throws Exception{
        ...
        BCCluster bcCluster = (BCCluster)BlockChainQueueUtils.peek();
        logger.info("队列的元素为：{}",bcCluster.getClusterName());
        bcCluster = (BCCluster)BlockChainQueueUtils.peek();
    ...
    public void testQueueWait()throws Exception{
        ...
        for(int i=0;i<10;i++){
            ThreadPoolManager.newInstance().addExecuteTask(new Runnable() {
                ...
                public void run() {
                    try{
                        logger.info("获取任务队列长度{}",BlockChainQueueUtils.size());
